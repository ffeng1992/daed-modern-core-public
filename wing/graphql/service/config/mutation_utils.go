...
func Update(ctx context.Context, _id graphql.ID, inputGlobal global.Input) (*Resolver, error) {
	if err := inputGlobal.Validate(); err != nil {
		return nil, err
	}
	...
	globalText, err := compatconfig.MarshalGlobal(c.Global)
	if err != nil {
		return nil, err
	}
	// Update.
...
func Run(d *gorm.DB, noLoad bool) (n int32, err error) {
	...
	if noLoad {
		...
		defer cancel()
		err = dae.SubmitReload(ctx, dae.EmptyConfig)
		if err != nil {
	...
	outbounds, err := dae.NecessaryOutbounds(&c.Routing)
	if err != nil {
		return 0, err
	}
	var groups []db.Group
	...
	reloadCtx, cancelReload := context.WithTimeout(context.Background(), 90*time.Second)
	defer cancelReload()
	errReload := dae.SubmitReload(reloadCtx, c)
	if errReload != nil {
