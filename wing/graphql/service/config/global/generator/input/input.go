...
func (b *builder) Build() (string, error) {
	...
	b.WriteLine(0, `import "github.com/daeuniverse/dae-wing/compatconfig"`)
	...
	b.WriteLine(0, `}`)
	b.WriteMethodLine(1, `if i.SoMarkFromDae != nil { g.SoMarkFromDaeSet = true }`)
	b.WriteMethodLine(0, `}`)
	b.WriteMethodLine(0, `
func (i *Input) Marshal() (string, error) {
            if err := i.Validate(); err != nil { return "", err }
            g, err := compatconfig.DefaultGlobal()
            if err != nil { return "", err }
            i.Assign(&g)
            return compatconfig.MarshalGlobal(g)
}`)
	return b.st.String() + "\n" + b.me.String(), nil
}
