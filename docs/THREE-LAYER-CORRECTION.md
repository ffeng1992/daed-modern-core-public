...
The official daed service declares `Conflicts=dae.service`. Coexistence therefore requires a separate API-only unit rather than enabling both upstream default units. The bridge management entry must block or correctly take over daed's native run/reload operations, since official API-only mode can acknowledge an internal reload without launching a DAE datapath. Until that contract is proven, the native page may serve only as a configuration editor and a separate bridge page must control DAE.
